...
export default function FactoryFinderOutput({ output, animate = false, isMobile = false }) {
              ...
              {images.map((img, i) => {
                ...
                return (
                  <DelayedVisible key={i} delay={imgDelay} animate={animate} style={{ borderRadius: 6, overflow: 'hidden', flexShrink: 0 }}>
                    <img src={img} alt="" style={{ width: imgSize, height: imgSize, objectFit: 'cover', display: 'block' }} />
                  </DelayedVisible>
                );
              })}
            </div>
            {url && (
              <a href={url} target="_blank" rel="noreferrer" style={{ fontSize: isMobile ? 7 : 10, color: '#2563eb', wordBreak: 'break-all' }}>
