...
export default function DemoModal({ isOpen, onClose }) {
  ...
  return (
    <div
      onClick={onClose}
      style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(10,37,64,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 500, overflow: 'hidden', position: 'relative', maxHeight: '92vh', overflowY: 'auto' }}
      >
        {/* Header */}
        <div style={{ background: navy, padding: '24px 28px 20px', position: 'sticky', top: 0, zIndex: 1 }}>
          ...
          <button
            onClick={onClose}
            style={{ position: 'absolute', top: 18, right: 20, background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 22, cursor: 'pointer', lineHeight: 1, padding: 4 }}
          >×</button>
        </div>
          ...
          <div style={{ padding: '48px 28px', textAlign: 'center' }}>
            <div style={{ width: 48, height: 48, borderRadius: '50%', background: mist, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
              <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                <path d="M4 11l5 5 9-9" stroke={sky} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
        ...
        {status !== 'success' && (
          <form onSubmit={handleSubmit} style={{ padding: '20px 28px 28px' }}>
            <div className="db-modal-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <label style={{ fontSize: 11, color: slate, letterSpacing: '1px', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Name</label>
              ...
              <div>
                <label style={{ fontSize: 11, color: slate, letterSpacing: '1px', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Email</label>
            ...
            <div className="db-modal-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <label style={{ fontSize: 11, color: slate, letterSpacing: '1px', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Company</label>
                <input required type="text" placeholder="Company name" value={form.company} onChange={e => setForm({ ...form, company: e.target.value })} style={inputStyle} />
              </div>
              <div>
                <label style={{ fontSize: 11, color: slate, letterSpacing: '1px', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Your role</label>
                <input type="text" placeholder="e.g. Operations Manager" value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} style={inputStyle} />
              </div>
            </div>
                    ...
                    <div key={f.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: mist, borderRadius: 8 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0 }}>
                          <rect x="2" y="1" width="10" height="12" rx="1.5" stroke={sky} strokeWidth="1.2" />
                          <path d="M4.5 4.5h5M4.5 7h5M4.5 9.5h3" stroke={sky} strokeWidth="1" strokeLinecap="round" />
                        </svg>
            ...
            {/* Error banner */}
            {status === 'error' && (
              <div style={{ marginBottom: 14, padding: '10px 14px', background: '#FDF2F2', border: `1px solid ${red}22`, borderRadius: 8, fontSize: 13, color: red, lineHeight: 1.5 }}>
                Something went wrong. Please try again or email us at <strong>hello@deep-bridge.com</strong>.
              </div>
            )}
            ...
            <button
              ...
              style={{ width: '100%', background: status === 'submitting' ? '#8BCDE8' : sky, color: 'white', border: 'none', borderRadius: 8, padding: '13px', fontSize: 14, fontWeight: 500, cursor: status === 'submitting' ? 'default' : 'pointer', fontFamily: 'inherit', transition: 'background 0.2s' }}
            >
              {status === 'submitting' ? 'Sending...' : status === 'error' ? 'Try again' : 'Request a demo'}
            </button>
